(() => {
  ...
  /** Campo de KM com separador de milhar: 124535 -> 124.535 */
  function bindKm(el) {
    el.addEventListener('input', () => {
      const d = digits(el.value).slice(0, 7);
      el.value = d ? fmtInt(parseInt(d, 10)) : '';
    });
  }
  const parseKm = (el) => { const d = digits(el.value); return d ? parseInt(d, 10) : null; };
  ...
  const DEFAULTS = () => ({
    carro: {
      ...
      valor: 10000, modo: 'valor', combustivel: null, km: null, kmTroca: null,
    },
    moto: {
      kml: { gasolina: 35, etanol: 25 }, tanque: 12,
      preco: { gasolina: PRESETS.gasolina[1], etanol: PRESETS.etanol[1] },
      valor: 5000, modo: 'valor', combustivel: null, km: null, kmTroca: null,
    },
  });
  ...
  function syncInputs() {
    ...
    $('#kmAtual').value = c.km != null ? fmtInt(c.km) : '';
    $('#kmTroca').value = c.kmTroca != null ? fmtInt(c.kmTroca) : '';
    $('#kmTitulo').textContent = `KM do ${state.veiculo}`;
  ...
  function render() {
    ...
    if (c.km == null) {
      $('#heroLabel').textContent = 'Salve o KM atual para liberar a calculadora';
      $('#heroKm').textContent = '—';
      $('#heroSub').textContent = '';
      $('#miniText').textContent = 'Salve o KM para começar';
    }
    renderKm();
    $('#regHint').textContent = c.km != null ? `Será registrado com o KM atual: ${fmtInt(c.km)} km.` : '';
  ...
  /** Bloco de KM: formulário (edição) ou resumo (salvo); trava a calculadora sem KM */
  function renderKm() {
    const c = cfg();
    const saved = c.km != null;
    const editing = !saved || state.editKm;
    $('#kmForm').hidden = !editing;
    $('#kmResumo').hidden = editing;
    $('#kmCancelar').hidden = !saved;
    $('#kmHint').hidden = saved;
    $('#calcBody').classList.toggle('locked', !saved);
    $('#calcBody').inert = !saved;
    if (!saved) return;

    $('#kmShow').textContent = fmtInt(c.km);
    $('#trocaShow').textContent = c.kmTroca != null ? fmtInt(c.kmTroca) : '—';
    const st = $('#trocaStatus');
    st.className = 'troca-status';
    if (c.kmTroca == null) {
      st.textContent = 'Nenhuma troca definida. Toque em Alterar KM para marcar a próxima.';
    } else {
      const resta = c.kmTroca - c.km;
      if (resta < 0) { st.textContent = `Troca vencida há ${fmtInt(-resta)} km.`; st.classList.add('late'); }
      else if (resta === 0) { st.textContent = 'Chegou a hora da troca.'; st.classList.add('late'); }
      else if (resta <= 500) { st.textContent = `Atenção: faltam ${fmtInt(resta)} km para a troca.`; st.classList.add('warn'); }
      else st.textContent = `Faltam ${fmtInt(resta)} km para a troca.`;
    }
  }
  ...
  function bindEvents() {
    ...
    // KM fixo e próxima troca
    bindKm($('#kmAtual'));
    bindKm($('#kmTroca'));
    $$('#kmForm [data-mais]').forEach((b) => b.addEventListener('click', () => {
      const km = parseKm($('#kmAtual'));
      if (km == null) { toast('Digite o KM atual primeiro'); $('#kmAtual').focus(); return; }
      $('#kmTroca').value = fmtInt(km + parseInt(b.dataset.mais, 10));
    }));

    $('#kmSalvar').addEventListener('click', () => {
      const c = cfg();
      const km = parseKm($('#kmAtual'));
      const troca = parseKm($('#kmTroca'));
      if (km == null) { toast('Digite o KM atual'); $('#kmAtual').focus(); return; }
      if (c.km != null && km < c.km && !confirm(`O KM digitado (${fmtInt(km)}) é menor que o salvo (${fmtInt(c.km)}). Salvar mesmo assim?`)) return;
      if (troca != null && troca < km && !confirm('A próxima troca é menor que o KM atual. Salvar mesmo assim?')) return;
      c.km = km; c.kmTroca = troca;
      state.editKm = false;
      persist(); render(); checkTroca();
      toast('KM salvo');
      window.scrollTo({ top: 0, behavior: 'smooth' });
    });
    $('#kmEditar').addEventListener('click', () => {
      state.editKm = true; syncInputs(); renderKm(); $('#kmAtual').focus();
    });
    $('#kmCancelar').addEventListener('click', () => {
      state.editKm = false; syncInputs(); renderKm();
    });
  ...
  /** Aviso: troca (km) chegando ou vencida */
  function checkTroca() {
    const c = cfg(), v = state.veiculo;
    if (c.km == null || c.kmTroca == null) return;
    const resta = c.kmTroca - c.km;
    if (resta <= 0) {
      addNotif({
        key: `troca-${v}-${c.kmTroca}-vencida`,
        titulo: `Troca do ${v} ${resta === 0 ? 'chegou' : 'vencida'}`,
        texto: resta === 0
          ? `O KM atual (${fmtInt(c.km)}) chegou na troca marcada.`
          : `Passou ${fmtInt(-resta)} km da troca marcada (${fmtInt(c.kmTroca)} km).`,
      });
    } else if (resta <= 500) {
      addNotif({
        key: `troca-${v}-${c.kmTroca}-perto`,
        titulo: `Troca do ${v} chegando`,
        texto: `Faltam ${fmtInt(resta)} km para a troca (${fmtInt(c.kmTroca)} km).`,
      });
    }
  }
